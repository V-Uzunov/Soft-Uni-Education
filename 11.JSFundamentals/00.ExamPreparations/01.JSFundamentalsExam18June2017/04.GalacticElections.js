function elections(input) {
    ...
    for (const items of input) {
        if (!res.has(items.system)) {
            res.set(items.system, new Map());
        }    
        if (!res.get(items.system).has(items.candidate)) {
            res.get(items.system).set(items.candidate, 0);
        }
        let currentVotes = res.get(items.system).get(items.candidate);
        res.get(items.system).set(items.candidate, currentVotes + items.votes);
    }
